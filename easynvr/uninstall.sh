...
if [ "$(id -u)" -eq 0 ]; then
    ...
    if sudo -n true 2>/dev/null; then
        ...
        echo -e "  ${GREEN}****** Use the following method to execute the stop.sh script. ******${NC}"
        echo -e "  ${GREEN}****** 1.Switch to root user and run ./stop.sh ******${NC}"
        echo -e "  ${GREEN}****** 2.Try running sudo ./stop.sh command. ******${NC}"
